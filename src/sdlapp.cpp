...
NONSHARABLE_CLASS(CSDLAppUi) : public CAknAppUi, public MExitWait, MSDLObserver	       
	...
	public:
		~CSDLAppUi();
	public: 
		TBool SetupScreenOrientationL(TBool aSetOrientation);
		TBool LandScapeModeEnabled();
...
void CSDLAppUi::ConstructL()
{
	...
	iLandScapeMode = (ApplicationRect().Width() == 240) || (ApplicationRect().Width() == 360);
...
TBool CSDLAppUi::LandScapeModeEnabled()
{
    return iLandScapeMode;
}
...
TBool CSDLAppUi::SetupScreenOrientationL(TBool aSetOrientation)
{
    ...
    RFile file;
    _LIT(KFilePath, "app_params.txt");
    TInt err = file.Open(iEikonEnv->FsSession(), KFilePath ,EFileRead);
    if(err == KErrNone)
    {
	file.Read(cmd);
	file.Close();
    ...
    _LIT8(KMode2, "-landscape");
    TBool foundMode = EFalse;
    TBool aSwitch = !aSetOrientation;
    cmd.Zero();
    for(TInt i = 0; i < appParams->Count(); i++)
    {
	if(appParams->MdcaPoint(i).Compare(KMode1) == 0)
	{
	    iLandScapeMode = EFalse;
	    foundMode = ETrue;
	}

	else if(appParams->MdcaPoint(i).Compare(KMode2) == 0)
	{
	...
	{
	    cmd.Append(appParams->MdcaPoint(i));
	    cmd.Append(_L8(" "));
	}
    }
    ...
    if(!foundMode || !aSwitch)
    {
	cmd.Append(KMode2); // set landscape by default
    }

    if(aSwitch) // switch modes
    {
	if(iLandScapeMode) // landscape mode found
	    cmd.Append(KMode1); // set portrait
	else
	    cmd.Append(KMode2); // set landscape
				
	iLandScapeMode = !iLandScapeMode;
    }

    if(file.Replace(iEikonEnv->FsSession(), KFilePath, EFileWrite) == KErrNone)
    {
	file.Write(cmd);
	file.Close();
    }	
...
extern "C" const int SetupScreenOrientation()
{
    if(gAppUi != NULL)
    {
	return gAppUi->SetupScreenOrientationL(EFalse);
    }
    return 0;
}
...
extern "C" const int GetScreenOrientation()
{
    if(gAppUi != NULL)
    {
	return gAppUi->LandScapeModeEnabled(); // 1 if landscape
    }
    return 0;
}
#if 0
...
{
    TInt exe_flags = SDLEnv::EFlagsNone;
#ifdef USE_VIRTUALMOUSE
    exe_flags |= SDLEnv::EVirtualMouse;
#endif
